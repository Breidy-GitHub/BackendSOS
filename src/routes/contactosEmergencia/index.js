...
router.get('/contactos_emergenciaALL', (req, res) => {
    conexion.query('SELECT * FROM 	contactos_emergencia', function (error, results, fields) {
        if (error) {
            console.error('Error al obtener tipos de usuario: ' + error.message);
            res.status(500).json({ error: 'Error al obtener tipos de usuario' });
            return;
        }
        res.json({ contactos_emergencia: results }); // Enviar todos los tipos de usuario en una sola respuesta
    });
});
